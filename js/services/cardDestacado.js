...
export const cardDestacado = (destacados) => {
  ...
  const btnDer = document.querySelector('#card-principal-der');
  const btnIzq = document.querySelector('#card-principal-izq');

  function renderCard(index) {
    limpiarHTML(colCardPrincipal);

    const juego = destacados[index];
    ...
    <div class="card-img-overlay p-2 d-flex flex-column justify-content-between justify-content-md-around">
      ...
      <div class="row mb-md-3 ps-md-3">
        ...
        <div class=" text-center text-md-start mb-5  ps-md-4">
          <a href="/views/game.html?id=${id}" data-id="${id}" class="btn btn-light link-juego fw-bold fs-6 pb-1">
